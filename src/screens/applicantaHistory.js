        ...
        const fetchApplicants = async () => {
            try {
                const response = await fetch('https://skywayapi.ntechagent.com/applicantshistory', {
                    ...
                    body: JSON.stringify({ agentname: agentName }), // This is unconventional for GET
                });
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
          try {
            ...
            const response = await fetch(`https://skywayapi.ntechagent.com/deleteapplicanthistory/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`, // Include the token in the headers
                    'Content-Type': 'application/json' // Optional: specify the content type
                  },
                body: JSON.stringify({ agentname: agentName }), // Sending agentName as JSON
            });
